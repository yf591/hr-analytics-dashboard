...
**実装された分析内容：**
- **KPI概要カード**：全体離職率、平均年齢、平均勤続年数を含む主要指標を視覚的に表示
- **部門別離職率**：各部門の離職率を比較するバーチャートを提供
- **データセットプレビュー**：包括的なデータ概要（レコード数、特徴量数）とサンプルデータを表示
- **データ品質チェック**：欠損値や異常値の有無を自動チェック
...
**実装された分析内容：**
- **多次元離職要因分析**：
  - 年齢層別離職率：若年層や特定年齢層の離職傾向を視覚化
  - 勤続年数別離職率：新入社員（0-2年）と中堅社員（3-7年）など勤続期間別の離職パターン
  - 満足度要因分析：職務満足度、環境満足度、ワークライフバランスと離職の相関関係
  - 部門×役職別離職率：部門と役職のクロス集計による離職ホットスポットの特定
  - 残業有無による離職率比較：ワークロードと離職の関係性分析

- **SHAP分析による要因の影響度測定**：
  - 機械学習モデルの予測に対する各特徴量の寄与度を示すSHAP値を計算
  - 特徴量が離職確率にプラスに働くかマイナスに働くかを可視化
  - 個人レベルでの要因分析が可能で、高リスク従業員の離職理由が直観的に理解可能

- **離職コスト試算**：
  - 役職・職種別の採用コスト、トレーニングコスト、生産性損失を考慮した離職コスト計算
  - 年間予測離職者数に基づく財務インパクト分析
  - コスト削減シナリオの提示（離職率1%低下による費用削減額など）
...
今後実装予定の人員需要予測モデルでは、以下の特徴を予定しています：
...
### Q: 予測モデルの精度はどのくらいですか？
A: モデルの精度は使用するアルゴリズムとデータによって異なります。各モデルページには精度指標（離職予測ではAccuracyとAUC、給与予測ではR²と平均絶対誤差）が表示されます。一般的に、ランダムフォレストやXGBoostモデルが高い精度を示しています。デフォルトの離職予測モデル（ランダムフォレスト）では、通常87-90%の精度が得られます。

### Q: 予測モデルはどのように学習・更新されますか？
A: モデルは以下の手順で学習・更新されます：
1. データセットの70%を訓練データ、30%をテストデータに分割
2. 訓練データでモデルを学習し、テストデータで精度を評価
3. モデルは分析ページ上で手動で再学習することも、定期的に自動更新することも可能
4. 新しいデータが追加された際は、モデルの再学習が推奨されます
...
A: モデルの予測結果は、特徴量重要度グラフと併せて解釈すると有益です。例えば、離職予測でリスクが高い場合は、特徴量重要度が高い要因（残業、満足度など）に注目することで、具体的な対策を検討できます。また、SHAP値分析は各要因がどのように予測に影響しているかをさらに詳細に示します。

### Q: カスタムモデルを追加できますか？
A: はい、システムは拡張性を考慮して設計されています。`src/models/`ディレクトリに新しいモデルファイルを追加し、インターフェースを既存モデルと合わせることで、独自のモデルを統合できます。詳細は開発ドキュメントを参照してください。

### Q: 離職予測モデルでリスクが高いと判定された従業員にはどのように対応すべきですか？
A: 高リスク従業員への対応は慎重に行う必要があります：
1. 予測はあくまで確率であり、100%の確実性はないことを理解する
2. SHAP値分析などで個人ごとの離職リスク要因を把握する
3. 直接的な「離職リスク」の言及は避け、特定の課題（満足度、キャリア開発など）に焦点を当てた対話を行う
4. 組織レベルでは、共通するリスク要因に対する施策を検討する
