...
def show():
    ...
    with tab2:
        ...
        try:
            # 欠損値を除去
            valid_data = df[["YearsAtCompany", "MonthlyIncome"]].dropna()

            if len(valid_data) > 1:  # データが十分にある場合のみ実行
                X = valid_data[["YearsAtCompany"]].values
                y = valid_data["MonthlyIncome"].values
                ...
                # 回帰線用のデータを作成
                x_range = np.linspace(
                    valid_data["YearsAtCompany"].min(),
                    valid_data["YearsAtCompany"].max(),
                    100,
                )
                ...
                # 統計情報を追加表示
                st.info(f"📊 相関係数: {correlation:.3f}, 決定係数 R²: {r_squared:.3f}")

        except Exception as e:
            st.warning(f"回帰線の描画でエラーが発生しました: {str(e)}")
            # エラーが発生した場合は回帰線なしで表示を継続
        ...
        perf_hike = (
            df.groupby("PerformanceRating")["PercentSalaryHike"].mean().reset_index()
        )
        ...
        satisfaction_income = (
            df.groupby(selected_satisfaction)["MonthlyIncome"].mean().reset_index()
        )

        fig = px.bar(
            satisfaction_income,
            x=selected_satisfaction,
            y="MonthlyIncome",
    ...
    with tab3:
        ...
        gender_income = (
            df.groupby(["Gender", "JobLevel"])["MonthlyIncome"].mean().reset_index()
        )
        ...
        # 統計量の計算
        stats_df = (
            df.groupby(test_var)["MonthlyIncome"]
            .agg(["count", "mean", "std"])
            .reset_index()
        )
        ...
        if p_val < alpha:
            st.error(
                f"p値が{alpha}未満であり、グループ間に統計的に有意な給与差が存在します。"
            )
        else:
            st.success(
                f"p値が{alpha}以上であり、グループ間に統計的に有意な給与差は検出されませんでした。"
            )
        ...
        pivot_df = df.pivot_table(
            index=row_var, columns=col_var, values="MonthlyIncome", aggfunc="mean"
        )
    ...
    st.markdown("---")
    st.info(
        "このページでは、給与・報酬の分析と公平性の評価を提供しています。"
