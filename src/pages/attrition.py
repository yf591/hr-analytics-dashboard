...
def show():
    ...
    with tab1:
        ...
        fig.update_layout(
            width=1000,  # 幅をさらに拡大
            height=1000,  # 高さをさらに拡大
            font=dict(size=10)  # フォントサイズをさらに大きくする
        )
        
        # 数値のフォントサイズも調整
        fig.update_traces(
            textfont=dict(size=14)  # セル内の数値のフォントサイズを調整
        )
        
        st.plotly_chart(fig, use_container_width=False)  # use_container_widthをFalseに変更して指定サイズを維持
        
        # ヒートマップとフッターの間に十分なスペースを確保（マージンを増やす）
        st.markdown("<div style='margin-bottom:450px;'></div>", unsafe_allow_html=True)
